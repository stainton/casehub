...
const CLOSED_VERSIONS_KEY='casehub-closed-versions';
function loadClosedVersions(){try{const saved=JSON.parse(localStorage.getItem(CLOSED_VERSIONS_KEY));if(Array.isArray(saved))return new Set(saved)}catch{}return new Set(['main'])}
...
let openTasks=new Set(), closedTaskVersions=new Set(), closedFolders=new Set(), closedReqFolders=new Set(), closedReviewFolders=new Set(), closedVersions=loadClosedVersions(), mainlineTreeInitialized=false;
...
function locateCaseInTree(c){
  $('[data-view="cases"]')?.click();setSidebarCollapsed(false);closedVersions.delete(c.VersionID);
  for(let id=c.FolderID;id;){closedFolders.delete(`${c.VersionID}:${id}`);id=state.folders.find(f=>f.VersionID===c.VersionID&&f.ID===id)?.ParentID||'';}
  renderVersions();const row=[...$('#versions').querySelectorAll('.case-row')].find(el=>el.dataset.version===c.VersionID&&el.dataset.case===c.ID);
