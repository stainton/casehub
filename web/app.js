...
function renderTasks(){let box=$('#tasks');box.innerHTML=state.tasks.map(t=>`<div class="task"><div class="task-head">▸ ${esc(t.Name)} <small>(${t.CaseIDs.length})</small></div><div class="task-body hidden">${tree(t.VersionID,new Set(t.CaseIDs),t.ID)}</div></div>`).join('')||'<p class="meta" style="padding:15px">暂无测试任务</p>';box.querySelectorAll('.task-head').forEach(e=>e.onclick=()=>e.nextElementSibling.classList.toggle('hidden'));box.querySelectorAll('.task-body .folder-row').forEach(e=>e.querySelector('.chev').onclick=x=>{x.stopPropagation();e.classList.toggle('closed')});box.querySelectorAll('.task-case').forEach(e=>{e.onclick=()=>setFocus({type:'case',versionID:e.dataset.v,id:e.dataset.c,taskID:e.dataset.task});e.oncontextmenu=x=>menu(x,taskCaseMenu(e.dataset.v,e.dataset.c,e.dataset.task))})}
...
function bindReqTree(root){
  root.querySelectorAll('[data-req-folder]').forEach(e=>{e.querySelector('.chev').onclick=x=>{x.stopPropagation();e.classList.toggle('closed')};e.onclick=()=>setReqFocus({type:'folder',id:e.dataset.reqFolder});e.oncontextmenu=x=>menu(x,reqFolderMenu(e.dataset.reqFolder))});
...
function bindReviewTree(root){
  root.querySelectorAll('[data-review-folder]').forEach(e=>{e.querySelector('.chev').onclick=x=>{x.stopPropagation();e.classList.toggle('closed')};e.onclick=()=>setReqFocus({type:'reviewFolder',id:e.dataset.reviewFolder});e.oncontextmenu=x=>menu(x,reviewFolderMenu(e.dataset.reviewFolder))});
